...
import {ExampleFactoryTokenJoinAuto} from "../../src/examples/ExampleFactoryTokenJoinAuto.sol";
...
contract FullLifecycleIntegrationTest is BaseExtensionTest {
    ...
    function setUp() public {
        ...
        extensionTokenJoin = ExampleTokenJoin(
            factoryTokenJoin.createExtension(address(joinToken), WAITING_BLOCKS)
        );
    ...
    function test_FullLifecycle_FactoryToExtension() public view {
        // 验证 factory 注册
        assertTrue(center.existsFactory(address(token), address(factoryTokenJoin)));
        assertTrue(
        ...
        assertTrue(factoryTokenJoinAuto.exists(address(extensionTokenJoinAuto)));
    ...
    function test_StateConsistency_AcrossRounds() public {
        ...
        // 验证账户快照独立
        address[] memory accounts0 = extensionTokenJoinAuto.accountsByRound(round0);
        address[] memory accounts1 = extensionTokenJoinAuto.accountsByRound(round1);
