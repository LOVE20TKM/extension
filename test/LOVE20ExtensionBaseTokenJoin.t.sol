...
import {LOVE20ExtensionBaseTokenJoin} from "../src/LOVE20ExtensionBaseTokenJoin.sol";
...
contract MockLOVE20ExtensionBaseTokenJoin is LOVE20ExtensionBaseTokenJoin {
    ...
    function joinedValueByAccount(address account) external view override returns (uint256) {
...
contract LOVE20ExtensionBaseTokenJoinTest is BaseExtensionTest {
    ...
    function test_Join_Success() public {
        ...
        // 验证 joinInfo
        (uint256 joinedAmount, uint256 joinedBlock, uint256 exitableBlock) = extension.joinInfo(user1);
    ...
    function test_Join_TransfersTokens() public {
        ...
        uint256 extensionBalanceBefore = joinToken.balanceOf(address(extension));
        
        vm.prank(user1);
        extension.join(amount, new string[](0));
        
        assertEq(joinToken.balanceOf(user1), userBalanceBefore - amount);
        assertEq(joinToken.balanceOf(address(extension)), extensionBalanceBefore + amount);
    }
    ...
    function test_JoinInfo_NotJoined() public view {
        (uint256 amount, uint256 joinedBlock, uint256 exitableBlock) = extension.joinInfo(user1);
        assertEq(amount, 0);
        assertEq(joinedBlock, 0);
        assertEq(exitableBlock, 0);
    }
