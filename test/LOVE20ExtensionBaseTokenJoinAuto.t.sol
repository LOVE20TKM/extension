...
import {LOVE20ExtensionBaseTokenJoinAuto} from "../src/LOVE20ExtensionBaseTokenJoinAuto.sol";
    ...
    LOVE20ExtensionBaseTokenJoinAuto
{
        ...
        returns (uint256 total, uint256[] memory scores)
    {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            (uint256 amount, , ) = this.joinInfo(_accounts[i]);
            scores[i] = amount;
            total += amount;
        }
    }
...
contract LOVE20ExtensionBaseTokenJoinAutoTest is BaseExtensionTest {
    ...
    function test_RewardByAccount_Proportional() public {
        ...
        mint.setActionReward(address(token), round0 + 1, ACTION_ID, totalReward);
