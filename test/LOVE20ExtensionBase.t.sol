...
contract MockLOVE20ExtensionBase is LOVE20ExtensionBase {
    ...
    function joinedValueByAccount(address) external pure override returns (uint256) {
        return 0;
    }
    ...
    function rewardByAccount(
        uint256,
        address
    ) public pure override(IExtensionReward, ExtensionReward) returns (uint256 reward, bool isMinted) {
        return (0, false);
    }
...
contract LOVE20ExtensionBaseTest is BaseExtensionTest {
    ...
    function test_Constructor_StoresFactory() public view {
        assertEq(extension.factory(), address(mockFactory), "Factory should be stored");
    }
    
    function test_Constructor_RetrievesCenter() public view {
        assertEq(extension.center(), address(center), "Center should be retrieved from factory");
    }
    
    function test_Constructor_NotInitialized() public view {
        assertFalse(extension.initialized(), "Should not be initialized at construction");
    }
    
    function test_Constructor_TokenAddressZero() public view {
        assertEq(extension.tokenAddress(), address(0), "Token address should be zero before init");
    }
    
    function test_Constructor_ActionIdZero() public view {
        assertEq(extension.actionId(), 0, "Action ID should be zero before init");
    }
    ...
    function test_Initialize_Success() public {
        ...
        // 通过 center 初始化
        center.initializeExtension(address(extension), address(token), ACTION_ID);
        
        // 验证状态
        assertTrue(extension.initialized(), "Should be initialized");
        assertEq(extension.tokenAddress(), address(token), "Token address should be set");
    ...
    function test_Interface_ILOVE20Extension() public view {
        ...
        assertEq(extension.joinedValueByAccount(user1), 0, "Should return 0 for any account");
    }
    ...
    function test_Integration_MultipleExtensions() public {
        // 创建多个扩展实例
        MockLOVE20ExtensionBase extension2 = new MockLOVE20ExtensionBase(address(mockFactory));
        ...
        submit.setActionInfo(address(token), ACTION_ID + 1, address(extension2));
        token.mint(address(extension2), 1e18);
        center.initializeExtension(address(extension2), address(token), ACTION_ID + 1);
